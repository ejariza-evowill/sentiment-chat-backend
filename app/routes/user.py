...
from fastapi import APIRouter, Depends, Cookie, Request, Response
from fastapi.responses import JSONResponse
from fastapi.exceptions import HTTPException

from app.config import EMAIL_REGEX, REFRESH_TOKEN_SECRET
from app.models import (
    User, UserCreate, Message, UserModel, Session, UserLogin
    )
from app.utils.databaseManager import get_db
from app.utils.SessionTokenManager import (
    create_access_token, create_refresh_token, get_payload_from_token
    )
...
def authenticate_user(request: Request, db=Depends(get_db)):
    ...
    session = db.query(Session).filter(Session.username == user.username).first()
...
def create_user(user: UserCreate,db = Depends(get_db)):
    ...
    if not user.username.isalnum():
        return JSONResponse(status_code=400, content={"detail":"Username must contain only alphanumeric characters"})
...
def user_login(userData: UserLogin, response: Response, db=Depends(get_db)):
    ...
    access_token, access_token_expires = create_access_token({"sub": user.username})
    refresh_token, refresh_token_expires = create_refresh_token({"sub": user.username})
    ...
    if session:
        ...
        new_session = Session(
            username=user.username, access_token=access_token, refresh_token=refresh_token,
            access_token_expires=access_token_expires, refresh_token_expires=refresh_token_expires
            )
        db.add(new_session)
    ...
    response.set_cookie(key="refresh_token", value=refresh_token, httponly=True, expires=refresh_token_expires)

    return {"detail":"Login successful"}

@router.post("/logout", response_model=Message, name="User Logout")
def user_logout(response: Response, user: User = Depends(authenticate_user), db=Depends(get_db)):
...
def refresh_token(response: Response, request: Request, db=Depends(get_db)):
    ...
    try:
        payload = get_payload_from_token(refresh_token, secret=REFRESH_TOKEN_SECRET)
        username = payload.get("sub")
    except Exception as e:
    ...
    session = db.query(Session).filter(Session.username == user.username).first()
    if not session:
        return JSONResponse(status_code=401, content={"detail":"Session not found, please login again"})
    ...
    if not session.refresh_token == refresh_token:
        return JSONResponse(status_code=401, content={"detail":"Invalid Credentials, please login again"})
